...
class CopilotQueryInput(BaseModel):
    query: str = Field(..., description="User's natural language analytical query")
    customer_id: Optional[str] = Field(None, description="Optional target customer ID context")


class CopilotQueryResponse(BaseModel):
    category: str = Field(..., description="Query category identifier")
    title: str = Field(..., description="Insight heading title")
    answer: str = Field(..., description="Comprehensive natural language answer")
    key_metrics: Dict[str, Any] = Field(default_factory=dict, description="Structured key-value metrics")
    suggested_actions: List[Dict[str, str]] = Field(default_factory=list, description="Actionable buttons/links")
