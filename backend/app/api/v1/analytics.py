...
from app.schemas.analytics import ActionAnalyticsRead, CopilotQueryInput, CopilotQueryResponse
...
@router.post(
    "/copilot",
    response_model=ResponseEnvelope[CopilotQueryResponse],
    status_code=status.HTTP_200_OK,
    summary="Ask Decision Copilot AI Query Assistant",
    description="Process natural language user queries regarding churn risk, customer profiles, Next-Best Actions, scenario simulations, SHAP explainability, and platform health.",
)
def ask_copilot(
    body: CopilotQueryInput,
    service: AnalyticsService = Depends(get_analytics_service),
):
    response = service.answer_copilot_query(query=body.query, customer_id=body.customer_id)
    return ResponseEnvelope(
        success=True,
        data=response,
